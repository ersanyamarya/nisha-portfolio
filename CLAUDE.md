...
- **Case study pages** are hand-written, one-off files under `src/pages/case-studies/*.tsx` (e.g. `flexera.tsx`, `spektrum.tsx`, `visionarai.tsx`) — there is no generated-page system for these (an earlier `gatsby-node.ts`/`details.tsx`/`ProjectsLookup` setup existed at one point but no longer does; `gatsby-node.ts` today only sets a webpack `@` alias). They share a component kit at `src/components/caseStudy/` (`CaseStudyShell`, `Eyebrow`, `MicroLabel`, `SectionHeading`, `ImageFrame`, `ZoomableImage`, `StatChip`, `PillTag`, etc., re-exported through its own barrel) plus a `diagrams/` subfolder for bespoke SVG diagrams. Screenshots live under `src/images/case-studies/<project>/`, imported directly (for full-quality zoom) and looked up a second time via the `useCaseStudyImages` hook (for an optimized, lazy-loaded inline version); pass both into `ImageFrame`. Per-project "skills applied" tags are shared between a case study's hero and its homepage card via `src/data/caseStudySkills.ts`.
- Homepage case-study cards are generated from a hardcoded `caseStudies` array in `src/sections/projects/index.tsx` (name, description, tags, stat, tone per project) — this is a separate source of truth from each case study page's own hero copy (only the tags are shared between the two; title/description/stat wording is written twice, once per context).
...
- `src/hooks/` has standalone hooks: `useForm` (a small reducer-based form/validation helper used by the contact form), `useScrollPosition` (drives `navBar.tsx`'s scroll-based styling), `useTheme` (light/dark theme state), and `useCaseStudyImages` (looks up optimized `gatsby-plugin-image` data for case-study screenshots by path, for use with `ImageFrame`).
